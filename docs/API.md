...
访问生成的 `/share/#TOKEN` 地址时，页面会立即跳转到 `/share/v1/latest?token=TOKEN`，由浏览器
直接显示该接口的紧凑 JSON 响应。其中 `data.message` 只包含最新邮件的 `uid`、`from`、`subject`、
`date`、压缩后的 `text`、`codes` 和 `partnerCodes`，不会返回重复的 `aliases` 或 `safeHtml`。生成
地址格式保持不变。
