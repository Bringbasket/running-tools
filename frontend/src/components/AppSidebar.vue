...
const latestBuild = computed(() => version.value?.latestVersion || version.value?.latestRevision?.slice(0, 8) || '可用')
        ...
        <span class="version-anchor" @click.stop>
          ...
          <div v-if="updateOpen" class="version-popover" @click.stop>
            ...
            <div class="version-details">
              <div><span>应用版本</span><strong>v{{ currentVersion }}</strong></div>
              <div v-if="currentRevision"><span>构建标识</span><strong>{{ currentRevision.slice(0, 8) }}</strong></div>
              <div v-if="version?.updateAvailable && version.latestVersion"><span>最新版本</span><strong>v{{ version.latestVersion }}</strong></div>
